...
func TestNFTController_GetDcnNFTMetadata(t *testing.T) {
	...
	t.Run("GET - dcn by node id decimal", func(t *testing.T) {
		...
		assert.Equal(t, "http://localhost/dcn/25188615033903404929663096463794904537890497498749865140845237535414961167487/image", gjson.GetBytes(body, "image").String())
		...
		assert.Equal(t, "Length", gjson.GetBytes(body, "attributes.2.trait_type").String())
		assert.Equal(t, "Nodehash", gjson.GetBytes(body, "attributes.3.trait_type").String())
	})
}
