...
type WebHooksControllerTestSuite struct {
	...
	autoAPISvc  *mock_services.MockAutoPiAPIService
}
...
func (s *WebHooksControllerTestSuite) SetupSuite() {
	...
	s.autoAPISvc = mock_services.NewMockAutoPiAPIService(s.mockCtrl)
	c := NewWebhooksController(&config.Settings{AutoPiAPIToken: token}, s.pdb.DBS, test.Logger(), s.autoAPISvc, s.ddDefIntSvc)
	app := fiber.New()
...
func (s *WebHooksControllerTestSuite) TestPostWebhookSyncCommand() {
	...
	s.autoAPISvc.EXPECT().UpdateJob(gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any()).Return(autopiJob, nil)
	s.autoAPISvc.EXPECT().UpdateState(gomock.Any(), gomock.Any()).Return(nil)
	...
	response, _ := s.app.Test(request, 10000)
...
func (s *WebHooksControllerTestSuite) TestPostWebhookRawCommand() {
	...
	response, _ := s.app.Test(request, 100000)
