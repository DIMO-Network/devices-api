...
func (udc *UserDevicesController) PostPairAutoPi(c *fiber.Ctx) error {
	...
	if recAddr, err := helpers.ECRecoverSol(hash.Bytes(), vehicleOwnerSig); err != nil {
		return err
	...
	if common.BytesToAddress(autoPiUnit.OwnerAddress.Bytes) != common.BytesToAddress(ud.R.VehicleNFT.OwnerAddress.Bytes) {
		...
		if recAddr, err := helpers.ECRecoverSol(hash.Bytes(), aftermarketDeviceSig); err != nil {
			return err
...
func (udc *UserDevicesController) UnpairAutoPi(c *fiber.Ctx) error {
	...
	recAddr, err := helpers.ECRecoverSol(hash[:], sigBytes)
	if err != nil {
		return err
	}
...
func (udc *UserDevicesController) PostClaimAutoPi(c *fiber.Ctx) error {
	...
	recUserAddr, err := helpers.ECRecoverSol(hash[:], userSig)
	if err != nil {
		return err
	}
	...
	recAmAddr, err := helpers.ECRecoverSol(hash[:], amSig)
	if err != nil {
		return err
	}
