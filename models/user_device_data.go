...
func (o *UserDeviceDatum) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(userDeviceDatumAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(userDeviceDatumPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert user_device_data, could not build conflict column list")
			}

			conflict = make([]string, len(userDeviceDatumPrimaryKeyColumns))
			copy(conflict, userDeviceDatumPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"devices_api\".\"user_device_data\"", updateOnConflict, ret, update, conflict, insert, opts...)
